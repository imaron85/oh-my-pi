...
- Top-level fleet sessions (and the main session) now see each other as `peer/<id>` rows in `hub list` and can message each other for resource coordination — each stays Main of its own agent tree; deliveries route into the target's own bus with reply-safe sender rewriting, capped by a per-pair wake budget against message loops.
